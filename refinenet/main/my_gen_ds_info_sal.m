...
function ds_info=my_gen_ds_info_sal(ds_config)



ds_dir=fullfile('/home/lab/lei', 'sal_trainval');

train_idx_file=fullfile(ds_dir, 'ImageSets/Main/saliency_train.txt');
fid=fopen(train_idx_file);
train_file_names=textscan(fid, '%s %s');
train_file_names = train_file_names{1};
fclose(fid);


val_idx_file=fullfile(ds_dir, 'ImageSets/Main/saliency_val.txt');
fid=fopen(val_idx_file);
val_file_names=textscan(fid, '%s %s');
...
ds_info.class_info=gen_class_info_sal();

ds_info.ds_name='sal_trainval';
