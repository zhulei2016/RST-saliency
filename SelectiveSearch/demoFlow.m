...
% Thresholds for hthte Felzenszwalb and Huttenlocher segmentation algorithm.
% Note that by default, we set minSize = k, and sigma = 0.8.
k = 200; % controls size of segments of initial segmentation. 
minSize = k;
sigma = 0.8;

% As an example, load video frame plus flow
im1 = imread('frame10.png');
load frame-flow2.mat

% Perform Selective Search
[colourIm] = Image2ColourSpace(im1, colorType);
...
[boxes blobIndIm blobBoxes hierarchy] = VideoFrame2HierarchicalGrouping(colourIm, flowIm, segmentIm, ... 
                                                                        sigma, k, minSize, colorType, simFunctionHandles);
boxes = BoxRemoveDuplicates(boxes);

% Show boxes
ShowRectsWithinImage(boxes(1:50, :), 5, 5, im1);

%% Show blobs which result from first similarity function
close all
hBlobs = RecreateBlobHierarchyIndIm(blobIndIm, blobBoxes, hierarchy{1});
ShowBlobs(hBlobs, 5, 5, im1);
...
%%
close all
hBlobs = RecreateBlobHierarchyIndIm(blobIndIm, blobBoxes, hierarchy{3});
ShowBlobs(hBlobs, 5, 5, im1);
