...
void mexFunction(int nl, mxArray *pl[], int nr, const mxArray *pr[])
{
	...
	const int avgDims[2] = { segSize, samplNum };	
	...
	pl[2] = mxCreateNumericArray(2, avgDims, mxDOUBLE_CLASS, mxREAL);
	double *segsAvg;	segsAvg = (double*)mxGetData(pl[2]);

	pl[3] = mxCreateNumericArray(2, imgDims, mxUINT32_CLASS, mxREAL);
	uint32 *salmap = (uint32*)mxGetData(pl[3]);
	pl[4] = mxCreateNumericArray(2, imgDims, mxUINT32_CLASS, mxREAL);
	uint32 *hitTime = (uint32*)mxGetData(pl[4]);
	...
	for (int s = 0; s < samplNum; ++s){
		for (int t = 0; t < nTreesEval; ++t){
			...
			for (int i = 0; i < segSize; ++i){
				segsAvg[i + s * segSize] += 
					segsOut[(s + t*samplNum) * segSize + i];
			}
