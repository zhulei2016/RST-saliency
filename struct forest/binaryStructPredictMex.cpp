...
void mexFunction(int nl, mxArray *pl[], int nr, const mxArray *pr[])
{
	...
	const int segDims[4] = { gtWidth, gtWidth, samplNum, nTreesEval };	// size macro for the structure prediction for each sample

	// create outputs
	pl[0] = mxCreateNumericArray(2, indDims, mxUINT32_CLASS, mxREAL);
	uint32 *ind = (uint32*)mxGetData(pl[0]);
	pl[1] = mxCreateNumericArray(4, segDims, mxUINT8_CLASS, mxREAL);
...
#pragma omp parallel for num_threads(nThreads)
	...
	for (int s = 0; s < samplNum; ++s){
		for (int t = 0; t < nTreesEval; ++t){
			...
			memcpy(segsOut + (s + t*samplNum)*gtWidth*gtWidth,
				segs + k*gtWidth*gtWidth,
				gtWidth*gtWidth*sizeof(uint8));

			if (nl > 2){
				uint32 kg = k*gtWidth*gtWidth;
