...
{
	// get inputs
	uint8 *slabel_1d = (uint8*)mxGetData(pr[0]);
	const mwSize *lblSize = mxGetDimensions(pr[0]);
	const int sz = (int)lblSize[0];	
	
	const int num_pix = (int)lblSize[1];
	const int num_tree = (int)lblSize[2];
	...
	const int sz_c = (int)mxGetScalar(mxGetField(para, 0, "sz_c"));
	const int sz_r = (int)mxGetScalar(mxGetField(para, 0, "sz_r"));
	...
	for (int s = 0; s < num_pix; ++s){
		for (int t = 0; t < num_tree; ++t){
			uint32 kg = sz * (s + t * num_pix);
			for (int x = 0, i = 0; x < sz_c; ++x){
				for (int y = 0; y < sz_r; ++y, ++i){
					int c1 = grid[s] + x;
					int r1 = grid[num_pix + s] + y;
					salmap[c1 * h + r1] += (double)slabel_1d[kg + i] *
						wt[s + t * num_pix];
					hitTime[c1 * h + r1]++;
				}
			}
