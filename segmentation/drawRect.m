...
% dest： 画好了的图像 
%---------------------------------------------------------------------- 
flag = 1;  %flag=1: 有缺口的框    %flag=2: 无缺口的框
%判断输入参数个数 
if nargin < 5, color = [255 255 0]; end 
if nargin < 4, lineSize = 1; end 
if nargin < 3, disp('输入参数不够 !!!'); return; end 
%判断框的边界问题 
[yA, xA, z] = size(src);
if isscalar(pt) == 1,
    [y1, x1] = ind2sub([yA, xA], pt);
else
    x1 = pt(1); y1 = pt(2); 
end
wx = wSize(1); wy = wSize(2); 
if x1>xA || y1>yA|| (x1+wx)>xA|| (y1+wy)>yA, disp('画的框将超过图像 !!!'); 
    return; 
end 
%如果是单通道的灰度图，转成3通道的图像 
