	...
	const double&				M)
{
	...
	for( int itr = 0; itr < 10; ++itr )
	{
		...
		for( int n = 0; n < numk; ++n )
		{
			...
			for( int y = y1; y < y2; ++y )
			{
				...
				for( int x = x1; x < x2; ++x, ++i )
				{
					...
					*/
					l_dist = m_lvec[i] - kseedsl[n];
					l_dist *= l_dist;
					a_dist = m_avec[i] - kseedsa[n];
					a_dist *= a_dist;
					b_dist = m_bvec[i] - kseedsb[n];
					b_dist *= b_dist;

					dist = l_dist + a_dist + b_dist;
		...
		*/
		memset(&sigmal.front(), 0, sigmal.size() * sizeof(double));
		memset(&sigmaa.front(), 0, sigmaa.size() * sizeof(double));
		memset(&sigmab.front(), 0, sigmab.size() * sizeof(double));
		memset(&sigmax.front(), 0, sigmax.size() * sizeof(double));
		memset(&sigmay.front(), 0, sigmay.size() * sizeof(double));
		memset(&clustersize.front(), 0, clustersize.size() * sizeof(double));
		//------------------------------------
	...
	const int&					colorspace)
{
	...
	//klabels.resize( sz, -1 );
	//--------------------------------------------------
	//Fuck!!! which clear the output!!!!
	//klabels = new int[sz];
	for( int s = 0; s < sz; s++ ) klabels[s] = -1;
    //--------------------------------------------------
    if(colorspace)//LAB, the default option
    {
...
{
	// just for debugging
	//mexPrintf( "Width: %d; Height: %d; \n Desired numbers: %d; Compactness: %f;\n test for 'klabels':%d; test for numlabels: %d\n", width,height, K, compactness,klabels[0], numlabels);
	//mexPrintf( "r[width]: %d, g[width]: %d; b[width]: %d\n", (ubuff[width] >> 16 & 0xff), (ubuff[width] >> 8 & 0xff), (ubuff[width] & 0xff));
    const int superpixelsize = 0.5+double(width*height)/double(K);
    DoSuperpixelSegmentation_ForGivenSuperpixelSize(ubuff,width,height,klabels,numlabels,superpixelsize,compactness, colorspace);
}
